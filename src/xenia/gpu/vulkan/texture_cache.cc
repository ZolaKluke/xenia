...
bool TextureCache::ConvertTexture(uint8_t* dest, VkBufferImageCopy* copy_region,
  ...
  uint32_t address =
      src.GetMipLocation(mip, &offset_x, &offset_y, &offset_z, true);
  if (!address) {
    return false;
  }
...
TextureExtent TextureCache::GetMipExtent(const TextureInfo& src, uint32_t mip) {
  ...
  if (mip == 0) {
    ...
    extent =
        TextureExtent::Calculate(format_info, dimension, mip_width, mip_height,
                                 mip_depth, src.is_tiled, false);
  }
  return extent;
}
...
uint32_t TextureCache::ComputeMipStorage(const TextureInfo& src, uint32_t mip) {
  uint32_t size =
      ComputeMipStorage(GetFormatInfo(src.format), src.dimension, src.width + 1,
